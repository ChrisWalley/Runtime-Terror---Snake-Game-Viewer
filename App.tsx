      ...
      if (viewerContext && gameServerUp) {
        ...
        if (gameState && gameState !== null && gameState.state >= 0) {
          ...
          for(i=0;i<3;i++){
            ...
            if(i==1){
              var obsRectsObj = parseCoords(gameState.obstacle1, obsStartIndex);
            }
            if(i==2){
              var obsRectsObj = parseCoords(gameState.obstacle2, obsStartIndex);
            }
            var obsRects = obsRectsObj.rects;
            for(j=0;j<obsRects.length ;j++){
              viewerContext.fillRect(startX + obsRects[j]['startX'] * blockSize, startY + obsRects[j]['startY'] * blockSize, obsRects[j]['width'] * blockSize, obsRects[j]['height'] * blockSize);
            }
          }
          ...
          for(i=0;i<4;i++){
            ...
            if(i==1){
              ...
              var snakeRectsObj = parseCoords(gameState.snake1.coords, snakeStartIndex);
            }
            if(i==2){
              viewerContext.fillStyle = gameColours.snake2;
              var snakeRectsObj = parseCoords(gameState.snake2.coords, snakeStartIndex);
            }
            if(i==3){
              viewerContext.fillStyle = gameColours.snake3;
              var snakeRectsObj = parseCoords(gameState.snake3.coords, snakeStartIndex);
            }
            var snakeRects = snakeRectsObj.rects;
            for (j = 0; j < snakeRects?.length; j++) {
              viewerContext.fillRect(startX + snakeRects[j]['startX'] * blockSize, startY + snakeRects[j]['startY'] * blockSize, snakeRects[j]['width'] * blockSize, snakeRects[j]['height'] * blockSize); //Draws coloured sqaure in viewer
            }
